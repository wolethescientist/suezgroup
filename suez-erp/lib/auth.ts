 ...
 * The built-in six are still there; an administrator can add more.
 */
...
export type SessionUser = {
   ...
   *
   * Carrying it on the session is what lets `can()` stay synchronous, so the
   * hundred-odd checks scattered through the app convert to a one-line call
   * instead of every one of them becoming async.
   */
  capabilities: string[];
};
...
/**
 * Anyone who can hold reports — a line manager decides their own team's leave
 * and timesheets whatever their role says, because that is a relationship, not
 * a permission. `leave.approve_any` is the capability for deciding beyond it.
 */
export const canApproveLeave = (u: SessionUser) =>
  canAny(u, "leave.approve_any", "leave.approve_executive", "timesheet.approve_any") || u.role === "manager";
