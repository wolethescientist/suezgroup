 ...
 * One place to look.
 *
 * ponytail: finding out whether anything needed you meant opening Tickets,
 * Quotes, Leads and Activities in turn, and the Notifications page — the only
 * thing that gathered anything — held a log of events that emptied itself the
 * moment it was read.
 */
export default async function InboxPage() {
  ...
  const [items, notes] = await Promise.all([
    inboxFor(me),
    sql<{
      id: number; title: string; body: string | null; href: string | null;
      kind: string; action_label: string | null; read_at: string | null; created_at: string;
    }>`
      select id, title, body, href, kind, action_label, read_at, created_at
        from notifications
       where user_id = ${me.id}
    ...
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_minmax(0,24rem)]">
        <Card>
          ...
          {items.length === 0 ? (
            <Empty title="Nothing outstanding" hint="Deposits running low, unanswered tickets and quotes to chase appear here." />
          ) : (
            <ul className="-mx-1 divide-y divide-line">
              {items.map((i) => (
                <li key={i.key} className="flex items-start gap-3 px-1 py-3">
                  ...
                  <Link
                    href={i.href}
                    className="shrink-0 rounded-lg bg-brand-50 px-2.5 py-1.5 text-xs font-bold whitespace-nowrap text-brand-700 hover:bg-brand-100"
                  >
                    {i.action}
                  </Link>
                </li>
        ...
        <Card>
          ...
          {notes.length === 0 ? (
            <Empty title="Nothing yet" hint="Assignments, deposits and ticket replies show up here." />
          ) : (
            <ul className="-mx-1 max-h-[38rem] divide-y divide-line overflow-y-auto">
              {notes.map((n) => (
                <li key={n.id} className={`flex items-start gap-2.5 rounded-lg px-1 py-3 ${n.read_at ? "" : "bg-brand-50/40"}`}>
