...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (storage_path) {
    const url = await signedUrl(storage_path, SIGNED_TTL, isInline(mime) ? undefined : name);
  ...
  return new Response(new Uint8Array(Buffer.from(data, "base64")), {
    headers: {
      "Content-Type": mime,
      "Content-Disposition": `${isInline(mime) ? "inline" : "attachment"}; filename="${name.replace(/"/g, "")}"`,
