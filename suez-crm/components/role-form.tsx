      ...
      <Row>
        <Field label="Name">
          <input name="name" required defaultValue={record?.name} className="field"
                 placeholder="e.g. Inside Sales" readOnly={record?.is_builtin} />
        </Field>
        {!record && (
          <Field label="Key" hint="Lowercase, no spaces. Stored on the employee record and cannot change later.">
            <input name="key" required className="field font-mono" placeholder="inside_sales" />
          </Field>
        )}
      </Row>

      <Field label="Description" hint="What this role is for, in a line. Shown on the Users screen.">
