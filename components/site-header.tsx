...
const NAV = [
  { href: "/companies", label: "Services & companies" },
...
export function SiteHeader() {
  ...
  return (
    <header className="site-header">
      ...
      <div className={`overflow-hidden rounded-[1.75rem] border-slate-line bg-slate/95 shadow-[0_18px_50px_rgb(21_36_39_/_0.1)] backdrop-blur-xl transition-[max-height,opacity,margin] duration-500 lg:hidden ${open ? "mt-2 max-h-[26rem] border opacity-100" : "mt-0 max-h-0 border-0 opacity-0"}`}>
        <nav aria-label="Mobile" className="flex flex-col px-5 py-4">
          {NAV.map((item, index) => (
            <Link key={item.href} href={item.href} className="flex items-baseline justify-between border-b border-slate-line py-4 font-display text-2xl transition-colors duration-200 last:border-0 hover:text-ember-ink">
              {item.label}<span className="font-sans text-[0.68rem] text-fg-slate-muted">0{index + 1}</span>
            </Link>
          ))}
          <Link href="/companies" className="btn btn-ember mt-4">Explore the group <span aria-hidden="true">↗</span></Link>
        </nav>
      </div>
    </header>
  );
